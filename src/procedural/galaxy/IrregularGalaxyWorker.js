...
self.onmessage = messageEvent => {
  ...
  const defaultBranchesNumber = THREE.MathUtils.randInt(galaxyParameters.irregular.branches.min, galaxyParameters.irregular.branches.max)
...
function _getGalaxyAttributesInRandomPosition (max, clusterSize, parameters, chosenColors, enforcedBranches) {
  ...
  const branches = enforcedBranches || THREE.MathUtils.randInt(parameters.irregular.branches.min, parameters.irregular.branches.max)
  const spin = THREE.MathUtils.randInt(parameters.irregular.spin.min, parameters.irregular.spin.max)
  const mixedColor = chosenColors.colorIn.clone()

  for (let i = 0; i < max; i++) {
    const i3 = i * 3
    const randomRadiusPosition = Math.random() * radius + Math.random()
    const spinAngle = (randomRadiusPosition * parameters.irregular.spinAmplitude) * spin
    const branchAngle = (i % branches) / branches * Math.PI * parameters.irregular.branchesAmplitude

    const x = Math.pow(
      Math.random(),
      parameters.irregular.randomnessPower
    ) * (Math.random() < 0.5 ? 1 : -1) * parameters.irregular.randomness * randomRadiusPosition
    ...
    mixedColor.lerpColors(
      ...
      (randomRadiusPosition / radius) + parameters.irregular.colorInterpolationAmplitude
    )
