...
self.onmessage = messageEvent => {
  ...
  const strangerThingsParameters = messageEvent.data.currentUniverse.matters.strangerthings.cyclic
...
function _getCyclicAttributesInRandomPosition (max, clusterSize, parameters) {
  ...
  for (let i = 0; i < max; i++) {
      ...
      positions[i3] = Math.sin(randomRadiusPosition) + x
      positions[i3 + 1] = y
      positions[i3 + 2] = Math.cos(randomRadiusPosition) + z
