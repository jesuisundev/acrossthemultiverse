...
function _getGalaxyAttributesInRandomPosition (max, clusterSize, parameters, chosenColors, enforcedBranches) {
  ...
  for (let i = 0; i < max; i++) {
    ...
    const spinAngle = (randomRadiusPosition * parameters.elliptical.spinAmplitude) * spin
    const branchAngle = (i % branches) / branches * Math.PI * parameters.elliptical.branchesAmplitude
